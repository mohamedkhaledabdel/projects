class EventsController < ApplicationController
	...
	# GET /events/new
  	...
  	def add_post
    		@postAdd = EventPost.create(event_id: params[:id],
    			user_id: params[:user_id],text: params[:event_post][:text])		
    		redirect_to :action => "show", :id => params[:id]
	  end

	  def add_comment
    		@commentAdd = EventComment.create(event_id: params[:id], 
    			post_id: params[:post_id], user_id: params[:user_id],
    			text: params[:event_comment][:text])
